    ...
    case "fetch_page": {
      try {
        const res = await fetch(args.url, {
          headers: { "User-Agent": "Mozilla/5.0 (compatible; DagrAI/1.0)" },
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          while (turnCount < maxTurns) {
            ...
            if (toolCalls.length > 0) {
              ...
              controller.enqueue(encoder.encode(
                `data: {"dagrai_tool": ${JSON.stringify({ name: lastCall.name, args: lastCall.args, result: lastCall.result.slice(0, 200) + "..." })}}\n\n`
              ));
            }
            ...
            if (msg?.tool_calls?.length) {
              for (const tc of msg.tool_calls) {
                ...
                controller.enqueue(encoder.encode(
                  `data: {"dagrai_tool_call": ${JSON.stringify({ name: funcName, args: funcArgs })}}\n\n`
                ));
                ...
                controller.enqueue(encoder.encode(
                  `data: {"dagrai_tool_result": ${JSON.stringify({ name: funcName, result: result.slice(0, 500) })}}\n\n`
                ));
          ...
          if (toolCalls.length > 0) {
            controller.enqueue(encoder.encode(
              `data: {"dagrai_reasoning": ${JSON.stringify(toolCalls.map(tc => `🔧 ${tc.name}(${Object.values(tc.args).join(", ")})`).join("\n"))}}\n\n`
            ));
          }
