...
export default function GuestChat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("sonixai_guest_messages");
      if (saved) {
  ...
  function saveMessages(msgs: Message[]) {
    try {
      localStorage.setItem("sonixai_guest_messages", JSON.stringify(msgs.slice(-50))); // keep last 50
    } catch {}
  }
  ...
  return (
    <div className="d-flex flex-column" style={{ height: "calc(100vh - 56px)" }}>
      ...
      >
        <span style={{ color: "rgba(255,255,255,0.3)" }}>guest@sonixai:~$</span>{" "}
        You&apos;re trying SonixAI — messages aren&apos;t saved.{" "}
        ...
        {messages.length === 0 && (
          ...
          >
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.85rem", textAlign: "center" }}>
              ...
              <div style={{ color: "rgba(255,255,255,0.18)" }}>SonixAI Terminal</div>
